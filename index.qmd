...
book_counts_by_genre %>%
  ...
  labs(title="I like to read books across different genres",
       subtitle = glue("Seems like <span style='color:#FF00FF'>**\"{genre_w_most_books1}\" and \"{genre_w_most_books2}\"**</span> are the genres I like the most!")) +
...
books %>%
  ...
  tab_header(title="Amit's book list",
             subtitle=glue("{nrow(books)} books, {length(genres)} genres, {max(books$year_read_or_purchased)-min(books$year_read_or_purchased)} years")) %>%
  tab_style(
